...
// constructor 2 when cluster size given, A.Menon
Foam::coarseMgMeshLevel::coarseMgMeshLevel(const mgMeshLevel& fineLevel, int Min, int Max)
:
    fineLevel_(fineLevel),
    // Sizes
    nCells_(0),
...
{
    // added by Menon
    fineLevel_.child(Min, Max);
    calcLevel();
}
// * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * * //

const Foam::labelList& Foam::coarseMgMeshLevel::faceRestrictAddr() const
{

    return faceRestrictAddr_;
}
