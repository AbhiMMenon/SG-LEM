...
(
    debug::optimisationSwitch("mgMinClusterSize", 4)
);

Foam::label Foam::mgMeshLevel::mgMaxClusterSize_
(
    debug::optimisationSwitch("mgMaxClusterSize", 4)
);
*/// salman
...
void Foam::mgMeshLevel::makeChild() const
{
    ...
    // cluster Abhi
    label mgMinClusterSize_ = 1;
    label mgMaxClusterSize_ = 1;
...
/* ------------------------------------------------------------------------------
 * New version of makeChild that takes in cluster size information
 *
 * Modified By A.Menon, 3rd Oct
 *-----------------------------------------------------------------------------*/

void Foam::mgMeshLevel::makeChild(int Min, int Max) const
{
    // Compact addressing for cellCells
    labelList cellCells;
    labelList cellCellOffsets;
    ...
    options[0] = 4;                   // globular agglom
    options[1] = 6;                   // objective F3 and F2
    options[2] = 0;                 // debugging output level
    options[3] = nGeometricD();       // Dimensionality of the grid
    
    // cluster Abhi
    label mgMinClusterSize_ = Min;
    label mgMaxClusterSize_ = Max;
    // Output: cell to coarse clusted addressing
    label nCoarseCells = 0;
    child_.setSize(nCells());
    int nMoves = -1;
...
// modified A. Menon
const Foam::labelList& Foam::mgMeshLevel::child(int Min, int Max) const
{
    if (child_.empty())
    {
        makeChild(Min,Max);
